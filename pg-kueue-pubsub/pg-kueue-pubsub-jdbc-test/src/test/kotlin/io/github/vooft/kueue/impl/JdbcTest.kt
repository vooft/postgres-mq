...
class JdbcTest : IntegrationTest() {
    ...
    fun `should produce to multiple topics and consume from multiple topics`(): Unit = runBlocking {
        HikariDataSource(
            ...
            with(HappyPathTest) {
                KueuePubSub.jdbc(dataSource = it).happyPathTest()
            }
        }
    }
    ...
    fun `should send after transaction commit`(): Unit = runBlocking {
        ...
        val kueue = KueuePubSub.jdbc(dataSource = dataSource)
        ...
        try {
            ...
            val kueueTransactionConnection = JdbcKueueConnection(transactionConnection)

            // send transaction message 1
            kueue.publish(topic, txn1, kueueTransactionConnection)

            // send normal message in-between transaction ones
            kueue.publish(topic, during)
            ...
            kueue.publish(topic, txn2, kueueTransactionConnection)
            ...
            kueue.publish(topic, after, kueueTransactionConnection)
    ...
    fun `should not send after transaction rollback`(): Unit = runBlocking {
        ...
        try {
            ...
            kueue.publish(topic, txn, kueueTransactionConnection)
